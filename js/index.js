...
function CargaCombosAtributos(IDCategoria){
  ...
  fetch(`https://api.mercadolibre.com/categories/${IDCategoria}/attributes`, requestOptions)
    ...
    .then(result => {
      ...
      Respuesta.forEach(element => {
        element.tags.catalog_required == true || Object.entries(element.tags).length == 0 ?
        ...
        if(contadorAtributos < 37){
          ...
          if(element.values != undefined && element.values != null && element.values != "" && element.id != "MANUFACTURER" && element.id != "BRAND" && element.id != "MODEL"){
            ...
            `<div class="mt-3 form-group input-container col-12 col-md">
              <select id="${element.id}" class="form-control form-control-user custom-input ${Requerido}" onchange="CargarArrayAtributos('${element.id}')">
                <option value="null" selected>No aplica</option>
              </select>
              <label for="${element.id}" class="input-label">${element.name}${SimboloRequerido}</label>
            </div>`
            ...
            divAtributos.innerHTML += 
            `<div class="mt-3 form-group input-container col-12 col-md">
              <input type="text" class="form-control form-control-user custom-input ${Requerido}" id="${element.id}" placeholder="${element.id}" onchange="CargarArrayAtributos('${element.id}')">
              <label for="${element.id}" class="input-label">${element.name}${SimboloRequerido}</label>
            </div>`
          }
        }
